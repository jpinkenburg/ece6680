...
vel = np.array([1,0]) #travel 1 unit/sec in the x direction

while 1:
  try:
    for i in range(4):
      pos,_ = p.getBasePositionAndOrientation(modArr[i])
      F = np.array([xDesired[i]-pos[0],0,0])
      p.applyExternalForce(modArr[i],-1,F,[0,0,0],p.LINK_FRAME)
      #print(F)
    pos1,ort = p.getBasePositionAndOrientation(mod1)
    ...
    #print('a')
    sleep(tstep)
    xDesired += np.ones(4)*tstep*vel[0]
    yDesired += np.ones(4)*tstep*vel[1]
    print(xDesired[3])
    print(F[0])
    print(pos4[0])
