...
while time.time() - start_time < 10:
  ...
  try:
    ...
    plt.figure()
    colors = [[x/(len(bodyPos)-1),0,1] for x in range(len(bodyPos)-1)]
    for i in range(len(bodyPos)-1):
      plt.plot([bodyPos[i][0],bodyPos[i+1][0]],[bodyPos[i][1],bodyPos[i+1][1]],color=colors[i])
    plt.show()
    raise(EOFError)
...
for i in range(len(bodyPos)-1):
  plt.plot([bodyPos[i][0],bodyPos[i+1][0]],[bodyPos[i][1],bodyPos[i+1][1]],color=twocolors[i,:3],linewidth=2)
plt.xlabel("Body X Position")
plt.ylabel("Body Y Position")
plt.show()
# raise(EOFError)
